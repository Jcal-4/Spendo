...
The ChatKit SDK enables a custom backend integration where all chat requests are handled by your Django server. This guide covers:

- Creating a memory store for thread persistence
- Implementing the ChatKit server
- Setting up Django endpoints
- Configuring CORS for ChatKit headers
...
**Important Notes:**
- `request.body` is a property, not a method - use `request.body` not `request.body()`
...
**Critical:** Without these headers, you'll get CORS errors when ChatKit tries to send requests with its custom headers.
...
- `api.url`: Points to your Django backend's ChatKit endpoint
...
### Error: generate_thread_id is async

**Symptom:** `'bytes' object is not callable` or coroutine issues

**Solution:** `generate_thread_id` and `generate_item_id` must be **synchronous** methods:
```python
def generate_thread_id(self, context: dict[str, Any] | None = None) -> str:
    # NOT async def
```
